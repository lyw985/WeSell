...
<script type="text/javascript">
	$(function() {
		...
		$("#addButton").click(function(){
			$("#divDialog").load('${commonMapper.rootPath}/module/new.do',null,function(){
				$("#moduleForm").validationEngine({ promptPosition : "topRight" });
				$("#divDialog").dialog({ 'width': 300,'height': 300,'title': '子系统新增',
					'buttons':{
						'保存':function(){
							if($(this).find('#moduleForm').validationEngine("validate")){
								$(this).find('#moduleForm').ajaxSubmit({ 
							        dataType:  'json',
							        type: 'POST',
					...
					beforeClose:function(event, ui) {
						$(this).find('#moduleForm').validationEngine("hideAll");
					},
		...
		$(".btnModify").click(function(){
			$("#divDialog").load('${commonMapper.rootPath}/module/modify/'+$(this).attr('rel')+'.do',null,function(){
				$("#moduleForm").validationEngine({ promptPosition : "topRight" });
				$("#divDialog").dialog({title:'子系统修改',resizable: false,height:300,width:300,
					'buttons':{
						'保存':function(){
							if($(this).find('#moduleForm').validationEngine("validate")){
								$(this).find('#moduleForm').ajaxSubmit({ 
		...
		$("#deleteButton").click(function() {
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					...
					$.post("${commonMapper.rootPath}/module/delete.do", ids.substring(1), function(data) {
						if (data.flag) {
		...
		$(".singleDelete").click(function(){
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/module/delete.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${commonMapper.rootPath}/module/query.do" method="post">
	...
	<!-- --------------------信息记录列表-------------------- -->
	<div class="listDiv">
		<table class="tabCls" cellpadding="0" cellspacing="0" width="100%">
			<tr>
			...
			<c:choose>
				<c:when test="${pageData.data != null && fn:length(pageData.data) > 0}">
					<c:forEach items="${pageData.data }" var="module" varStatus="vs">
						<tr ${vs.index % 2 == 0 ? "class='odd'" : "class='even'"}>
							<td><input type="checkbox" class="selOne" value="${module.id }"></td>
							<td><c:out value="${module.name }"></c:out></td>
							<td><c:out value="${module.code }"></c:out></td>
							<td><fmt:formatDate value="${module.createTime}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate></td>
							<td><c:out value="${module.description }"></c:out></td>
							<td>
								<a href="javascript:void(0)" rel="${module.id}" class="btnModify">修改</a>
								<a href="javascript:void(0)" rel="${module.id}" class="singleDelete">删除</a>
							</td>
						</tr>
					</c:forEach>
