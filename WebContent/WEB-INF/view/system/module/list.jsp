...
<script type="text/javascript">
	$(function() {
		...
		$("#addButton").click(function(){
			$("#divDialog").load('/module/new.do',null,function(){
		...
		$(".btnModify").click(function(){
			$("#divDialog").load('/module/modify/'+$(this).attr('rel')+'.do',null,function(){
				$("#moduleForm").validationEngine({ promptPosition : "topRight" });
		...
		$("#deleteButton").click(function() {
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					...
					$.post("${pageContext.request.contextPath}/module/delete.do", ids.substring(1), function(data) {
						if (data.flag) {
		...
		$(".singleDelete").click(function(){
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					$.post('/module/delete.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${pageContext.request.contextPath}/module/query.do" method="post">
