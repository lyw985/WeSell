...
<script type="text/javascript">
	$(function() {
		...
		$("#addButton").click(function(){
			$("#divDialog").load('${commonMapper.rootPath}/blsh/content/new.do',null,function(){
		...
		$(".btnModify").click(function(){
			$("#divDialog").load('${commonMapper.rootPath}/blsh/content/modify/'+$(this).attr('rel')+'.do',null,function(){
				$("#blshContentForm").validationEngine({ promptPosition : "topRight" });
		...
		$("#deleteButton").click(function() {
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					...
					$.post("${commonMapper.rootPath}/blsh/content/delete.do", ids.substring(1), function(data) {
						if (data.flag) {
		...
		$(".singleDelete").click(function(){
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/blsh/content/delete.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".passVersion").click(function(){
			...
			jConfirm('确认执行生效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/blsh/content/pass.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".refuseVersion").click(function(){
			...
			jConfirm('确认执行失效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/blsh/content/refuse.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${commonMapper.rootPath}/blsh/content/query.do" method="post">
