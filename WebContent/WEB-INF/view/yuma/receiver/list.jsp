...
<script type="text/javascript">
	$(function() {
		...
		$("#addButton").click(function(){
			...
			$("#divDialog").load('${commonMapper.rootPath}/yuma/receiver/new.do',tmp.substring(1),function(){
		...
		$(".btnModify").click(function(){
			$("#divDialog").load('${commonMapper.rootPath}/yuma/receiver/modify/'+$(this).attr('rel')+'.do',null,function(){
				$("#yumaReceiverForm").validationEngine({ promptPosition : "topRight" });
		...
		$("#deleteButton").click(function() {
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					...
					$.post("${commonMapper.rootPath}/yuma/receiver/delete.do", ids.substring(1), function(data) {
						if (data.flag) {
		...
		$(".singleDelete").click(function(){
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/yuma/receiver/delete.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".setDefaultAddress").click(function(){
			...
			jConfirm('确认设为默认收货人吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/yuma/receiver/setDefaultAddress.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".unsetDefaultAddress").click(function(){
			...
			jConfirm('确认取消默认收货人吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/yuma/receiver/unsetDefaultAddress.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".orderListByReceiverId").click(function(){
			window.location.href="${commonMapper.rootPath}/yuma/order/list.do?receiver_id="+$(this).attr('rel');
		});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${commonMapper.rootPath}/yuma/receiver/query.do" method="post">
