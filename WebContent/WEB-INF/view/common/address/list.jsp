...
<script type="text/javascript">
	$(function() {
		...
		$("#addButton").click(function(){
			$("#divDialog").load('/common/address/new.do',null,function(){
		...
		$(".btnModify").click(function(){
			$("#divDialog").load('/common/address/modify/'+$(this).attr('rel')+'.do',null,function(){
				$("#provinceForm").validationEngine({ promptPosition : "topRight" });
		...
		$("#deleteButton").click(function() {
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					...
					$.post("${pageContext.request.contextPath}/common/address/delete.do", ids.substring(1), function(data) {
						if (data.flag) {
		...
		$(".singleDelete").click(function(){
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					$.post('/common/address/delete.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".beShadow").change(function(){
			...
			jConfirm('确认将选择的省份设置为本省份的本体吗？', '确认操作', function(result){
				if(result == true){
					$.post('/common/address/beShadow.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".beBody").click(function(){
			...
			jConfirm('确认设置为本体吗？', '确认操作', function(result){
				if(result == true){
					$.post('/common/address/beBody.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".showCity").click(function(){
			...
			jConfirm('需要展示【'+cityName+'】吗？', '确认操作', function(result){
				if(result == true){
					$.post('/common/address/showCity.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".hideCity").click(function(){
			...
			jConfirm('需要隐藏【'+cityName+'】吗？', '确认操作', function(result){
				if(result == true){
					$.post('/common/address/hideCity.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${pageContext.request.contextPath}/common/address/query.do" method="post">
