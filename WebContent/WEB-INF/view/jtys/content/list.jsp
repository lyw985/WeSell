...
<script type="text/javascript">
	$(function() {
		...
		$("#addButton").click(function(){
			$("#divDialog").load('/jtys/content/new.do',null,function(){
		...
		$(".btnModify").click(function(){
			$("#divDialog").load('/jtys/content/modify/'+$(this).attr('rel')+'.do',null,function(){
				$("#jtysContentForm").validationEngine({ promptPosition : "topRight" });
		...
		$("#deleteButton").click(function() {
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					...
					$.post("${pageContext.request.contextPath}/jtys/content/delete.do", ids.substring(1), function(data) {
						if (data.flag) {
		...
		$(".singleDelete").click(function(){
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					$.post('/jtys/content/delete.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".passVersion").click(function(){
			...
			jConfirm('确认执行生效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('/jtys/content/pass.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".refuseVersion").click(function(){
			...
			jConfirm('确认执行失效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('/jtys/content/refuse.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${pageContext.request.contextPath}/jtys/content/query.do" method="post">
