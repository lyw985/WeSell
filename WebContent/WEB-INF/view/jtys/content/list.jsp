...
<script type="text/javascript">
	$(function() {
		...
		$("#addButton").click(function(){
			$("#divDialog").load('${commonMapper.rootPath}/jtys/content/new.do',null,function(){
				$("#jtysContentForm").validationEngine({ promptPosition : "topRight" });
				$("#divDialog").dialog({ 'width': 400,'height': 400,'title': '信息新增',
					'buttons':{
						'保存':function(){
							if($(this).find('#jtysContentForm').validationEngine("validate")){
								$(this).find('#jtysContentForm').ajaxSubmit({ 
							        dataType:  'json',
							        type: 'POST',
					...
					beforeClose:function(event, ui) {
						$(this).find('#jtysContentForm').validationEngine("hideAll");
					},
		...
		$(".btnModify").click(function(){
			$("#divDialog").load('${commonMapper.rootPath}/jtys/content/modify/'+$(this).attr('rel')+'.do',null,function(){
				$("#jtysContentForm").validationEngine({ promptPosition : "topRight" });
		...
		$("#deleteButton").click(function() {
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					...
					$.post("${commonMapper.rootPath}/jtys/content/delete.do", ids.substring(1), function(data) {
						if (data.flag) {
		...
		$(".singleDelete").click(function(){
			...
			jConfirm('确认要删除记录吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/jtys/content/delete.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".passVersion").click(function(){
			...
			jConfirm('确认执行生效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/jtys/content/pass.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".refuseVersion").click(function(){
			...
			jConfirm('确认执行失效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/jtys/content/refuse.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${commonMapper.rootPath}/jtys/content/query.do" method="post">
				...
				<tr>
					...
					<td>
						<select id="type" name="type" value="${type}">
							<option value="">--选择消息类型--</option>
							<c:forEach items="${jtysContentTypes }" var="item">
		...
		<table class="tabCls" cellpadding="0" cellspacing="0" width="99%">
			...
			<c:choose>
				<c:when test="${pageData.data != null && fn:length(pageData.data) > 0}">
					<c:forEach items="${pageData.data }" var="jtysContent" varStatus="vs">
						<tr ${vs.index % 2 == 0 ? "class='odd'" : "class='even'"} >
							<td><input type="checkbox" class="selOne" value="${jtysContent.id}"></td>
							<td style="text-align: left;">
								<c:choose>
									<c:when test="${empty jtysContent.content}">
										<img src="${jtysContent.picUrl }" height="50px" title="点击查看大图" onclick="window.open('${jtysContent.picUrl }')"/>
										<a href="${jtysContent.url }" target="_blank" style="font-size: 18px;" title="点击查看原文">${jtysContent.title }</a>
									</c:when>
									<c:otherwise>
										<div style="width:500px; overflow:hidden; white-space:nowrap; text-overflow:ellipsis; -o-text-overflow:ellipsis; -icab-text-overflow:ellipsis; -khtml-text-overflow:ellipsis; -moz-text-overflow:ellipsis; -webkit-text-overflow:ellipsis; " title ="${jtysContent.content}">${jtysContent.content}</div>
									</c:otherwise>
								</c:choose>
							</td>
							<td><fmt:formatDate value="${jtysContent.createTime}"  pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate></td>
							<td>${jtysContent.user.name}</td>
							<td>${jtysContent.typeStr}</td>
							<td>
								<c:if test="${jtysContent.status==0}"><font color="blue">${jtysContent.statusStr}</font></c:if>
								<c:if test="${jtysContent.status==1}"><font color="green">${jtysContent.statusStr}</font></c:if>
								<c:if test="${jtysContent.status==2}"><font color="red">${jtysContent.statusStr}</font></c:if>
							</td>
							<td>
								<c:if test="${jtysContent.status!=1}">
									<a href="javascript:void(0)" rel="${jtysContent.id}" class="passVersion">通过</a>
								</c:if>
								<c:if test="${jtysContent.status!=2}">
									<a href="javascript:void(0)" rel="${jtysContent.id}" class="refuseVersion">不通过</a>
								</c:if>
								<a href="javascript:void(0)" rel="${jtysContent.id}" class="btnModify">修改</a>
								<a href="javascript:void(0)" rel="${jtysContent.id}" class="singleDelete">删除</a>
							</td>
						</tr>
					</c:forEach>
