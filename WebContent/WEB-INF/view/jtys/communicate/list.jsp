	...
	$(function() {
		...
		$(".passVersion").click(function(){
			...
			jConfirm('确认执行生效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/jtys/communicate/pass.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
		...
		$(".refuseVersion").click(function(){
			...
			jConfirm('确认执行失效操作吗？', '确认操作', function(result){
				if(result == true){
					$.post('${commonMapper.rootPath}/jtys/communicate/refuse.do',tmp.substring(1),function(data){
						if(data.flag){
							jAlert(data.msg,'成功',function(){
								$("#queryForm").submit();
							});
...
<body>
	...
	<!-- --------------------查询表单-------------------- -->
	<div class="ui-widget">
		<form:form id="queryForm" action="${commonMapper.rootPath}/jtys/communicate/query.do" method="post">
					...
					<c:forEach items="${pageData.data }" var="jtysCommunicate" varStatus="vs">
						<tr ${vs.index % 2 == 0 ? "class='odd'" : "class='even'"} >
							...
							<td>
								...
								<c:if test="${jtysCommunicate.status==2}"><font color="red">${jtysCommunicate.statusStr}</font></c:if>
							</td>
							<td>
								<c:if test="${jtysCommunicate.status!=1}">
									<a href="javascript:void(0)" rel="${jtysCommunicate.id}" class="passVersion">通过</a>
								</c:if>
								<c:if test="${jtysCommunicate.status!=2}">
									<a href="javascript:void(0)" rel="${jtysCommunicate.id}" class="refuseVersion">不通过</a>
								</c:if>
								<a href="javascript:void(0)" rel="${jtysCommunicate.id}" class="btnModify">修改</a>
								<a href="javascript:void(0)" rel="${jtysCommunicate.id}" class="singleDelete">删除</a>
							</td>
						</tr>
					</c:forEach>
