    ...
    public void doFilter(ServletRequest arg0, ServletResponse arg1, FilterChain arg2) throws IOException,
        ...
        if (session == null || session.getAttribute(PermissionConstants.CONSTANT_PARAM_USER) == null) {
            if (StringUtil.isNotBlank(url)
                && (url.indexOf("home") < 0 && url.indexOf("login") < 0 && url.indexOf("Register") < 0
                    && url.indexOf("register") < 0 && url.indexOf("api") < 0)) {
                response.sendRedirect(CommonPropUtil.getCommonMapper().getIndexPath());
                return;
            }
        }
